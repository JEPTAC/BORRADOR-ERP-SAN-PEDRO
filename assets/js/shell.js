  ...
  const ERP = {
    init() {
      ...
      document.addEventListener('click', e => {
        if (document.body.classList.contains('sidebar-open') && !e.target.closest('.sidebar') && !e.target.closest('[data-menu-toggle]')) {
    ...
    closeAll() {
      document.querySelectorAll('.modal-backdrop:not(.hidden), .drawer-backdrop:not(.hidden), .drawer:not(.hidden)').forEach(el => el.classList.add('hidden'));
      document.body.style.overflow = '';
    },
    ...
    formatDate(value, opts = { day: '2-digit', month: 'short', year: 'numeric' }) {
      if (!value) return '—';
